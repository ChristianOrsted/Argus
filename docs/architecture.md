...
- 文件路径沙箱限制。
- 执行器采用默认拒绝的能力沙箱：`run_shell` 不调用宿主 Shell，只实现少量受限命令。
- 文件执行前再次检查路径、链接和挂载点，避免 Guardian 被绕过后直接越界。
- HTTP(S) 抓取只连接已解析并固定的公网 IP，重定向目标必须重新通过 SSRF 校验。
...
- 阶段 1：连续重复同一工具超过阈值即 `FLAG`。
- 后续：基于良性序列建立 n-gram/Markov 基线，输出异常分数。
