...
Dashboard 会把每条 DeepSeek 生成样本拆成“攻击目标、用户请求、工具调用、污点来源、风险点、危险解释”，并把 Guardian 结果拆成“总状态、四层 Verdict、漏拦截状态、DeepSeek 分析结果、自适应规则写入结果”。如果某条红队样本期望 `block/flag` 但 Guardian 放行，页面可直接点击“调用 DeepSeek 分析并优化规则”，让模型分析漏拦截原因，再把建议约束为 `adaptive_rules.py` 支持的字段规则后重评估。
